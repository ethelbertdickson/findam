...
export interface UserRequest { id: string; type: UserRequestType; title: string; description: string; status: string; criteria?: Record<string, unknown> | null; city?: { name: string } | null; serviceCategory?: ProfessionalCategory | null; }
...
export async function fetchMyRequests() { const { data } = await apiClient.get<UserRequest[]>('/requests/mine'); return data; }
