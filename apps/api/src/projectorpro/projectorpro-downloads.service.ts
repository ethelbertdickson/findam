...
export class ProjectorProDownloadsService {
  ...
  async publicStats() {
    ...
    return { product: PRODUCT, downloadsStarted: started, downloadsCompleted: completed };
  }

  async streamLatest(request: Request, response: Response) {
    const filePath = resolve(this.config.get<string>('PROJECTORPRO_DOWNLOAD_FILE') ?? '/var/lib/findam/downloads/projectorpro/ProjectorPro-latest.zip');
    if (!existsSync(filePath) || !statSync(filePath).isFile()) throw new NotFoundException('This download is not available yet.');
    ...
    const event = await this.prisma.downloadEvent.create({ data: { product: PRODUCT, version, platform: request.headers['x-platform']?.toString().slice(0, 40), ipHash, countryCode, continent: continentFor(countryCode), userAgent: request.headers['user-agent']?.slice(0, 500), referrer: request.headers.referer?.slice(0, 500) } });
