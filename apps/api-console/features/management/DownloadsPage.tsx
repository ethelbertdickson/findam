...
export function DownloadsPage({ recoverSession, csrfToken }: { recoverSession: () => Promise<boolean>; csrfToken: string }) {
  ...
  return <div className="space-y-6 p-5 sm:p-8">
    <ConsoleHeaderSlot>
      <select className="h-9 rounded-md border bg-background px-2 text-sm" aria-label="Release application" value={app} onChange={(event) => setApp(event.target.value)}><option value="projectorpro">ProjectorPro</option><option value="findam">Findam</option><option value="marketlist">Marketlist</option></select>
      <select className="h-9 rounded-md border bg-background px-2 text-sm" aria-label="Release platform" value={platform} onChange={(event) => setPlatform(event.target.value)}><option value="windows">Windows</option><option value="linux">Linux</option><option value="macos">macOS</option></select>
      <input ref={fileInput} type="file" accept={platform === 'windows' ? '.exe,.msi,.zip' : platform === 'linux' ? '.deb,.AppImage,.tar.gz,.zip' : '.dmg,.pkg,.zip'} className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadFile(file); }} />
      <Button size="sm" onClick={() => void onUpload()} disabled={uploading}><Upload />{uploading ? 'Uploading…' : 'Upload release'}</Button>
