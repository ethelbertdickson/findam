...
export function DownloadsPage({ recoverSession, csrfToken }: { recoverSession: () => Promise<boolean>; csrfToken: string }) {
  ...
  async function onUpload() {
    ...
    try { await uploadAppRelease(app, file, csrfToken); if (fileInput.current) fileInput.current.value = ''; }
  ...
  return <div className="space-y-6 p-5 sm:p-8">
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><Upload className="size-4 text-primary" />App release files</CardTitle><p className="text-sm text-muted-foreground">Upload a release into its application folder on the VPS. ProjectorPro ZIP files become the public latest download.</p></CardHeader><CardContent className="flex flex-wrap items-center gap-3"><select className="h-10 rounded-md border bg-background px-3 text-sm" value={app} onChange={(event) => setApp(event.target.value)}><option value="projectorpro">ProjectorPro</option><option value="findam">Findam</option><option value="marketlist">Marketlist</option></select><input ref={fileInput} type="file" className="max-w-full text-sm" /><Button onClick={() => void onUpload()} disabled={uploading}>{uploading ? 'Uploading…' : 'Upload release'}</Button></CardContent></Card>
